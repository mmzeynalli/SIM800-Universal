...
// ============================================================
void SIM800::writeln(char *out) {                                // This method accepts a character array as an input. "\r\n" is appended to the end of character array
    strcat(out, "\r\n");                                        // Let SoftwareSerial do the actual hard work of banging the outgoing cmd string bit-by-bit out to the SIM800 chip.
    simCom->print(out);                                         // No read() is called. As mostly all AT cmds cause the SIM800 chip to provide some kind of reply, 
}                                                               // the user runs the risk of missing them if they do not call read() after manually calling the write(char[]) method.
